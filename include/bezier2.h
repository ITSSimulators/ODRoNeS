...
public:
    bezier2();
    bezier2(const bezier2& b);
    bezier2& operator=(const bezier2& b);
    ~bezier2() override;
    void assignInputLaneToThis(const bezier2& b);

    // bezier2(const arr2 &p0, const arr2 &p1, const arr2 &p2);
    void set(const arr2 &p0, const arr2 &p1, const arr2 &p2);
    ...
    bool rootPx(scalar &t) const override;  ///< returns the value of t at which Bx' == 0; false if never.
    bool rootPy(scalar &t) const override;  ///< returns the value of t at which By' == 0; false if never.

    bool getTGivenXY(scalar &t, const arr2 &xy) const override;

    /*! return a new bezier curve that starts in t = 0 and ends in t: */
    bezier2 getStartingPart(scalar t) const;
    /*! return a new bezier curve that starts in t and ends in 1 */
    bezier2 getEndingPart(scalar t) const;
